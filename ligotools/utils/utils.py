...
def plots_PSD(det, pcolor, strain_whitenbp, template_match, **kwargs):

    """Function to plot the PSD of data. """
    
    # kwargs needed for the plotting
    time = kwargs["time"]
    timemax = kwargs["timemax"]
    tevent = kwargs["tevent"]
    SNR = kwargs["SNR"]
    fig_folder = kwargs["fig_folder"]
    eventname = kwargs["eventname"]
    plottype = kwargs["plottype"]
    template_fft = kwargs["template_fft"]
    datafreq = kwargs["datafreq"]
    d_eff = kwargs["d_eff"]
    freqs = kwargs["freqs"]
    data_psd = kwargs["data_psd"]
    fs = kwargs["fs"]
    ...
    plt.xlabel('Time since {0:.4f}'.format(timemax))
    plt.legend(loc='upper left')
    plt.savefig(fig_folder + eventname+"_"+det+"_SNR."+plottype)
    ...
    plt.savefig(fig_folder + eventname+"_"+det+"_matchtime."+plottype)
    ...
    plt.savefig(fig_folder + eventname+"_"+det+"_matchfreq."+plottype)
